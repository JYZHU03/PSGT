...
from torch_geometric.nn import GCNConv, SAGEConv, GATConv, DataParallel, TAGConv, GINConv, GINEConv
...
class Model(torch.nn.Module):
	def __init__(self, args, concat=False):
		...
		elif self.model == 'tagc':
			self.conv1 = TAGConv(self.num_features, self.nhid)
		elif self.model == 'gin':
			self.conv1 = GINConv(self.num_features, self.nhid)
		elif self.model == 'gine':
			self.conv1 = GINEConv(self.num_features, self.nhid)
...
parser.add_argument('--model', type=str, default='gat', help='model type, [gcn, gat, sage, tagc]')
